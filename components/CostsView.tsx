...
import { ICONS, CATEGORY_GROUPS } from '../constants';
...
const CostsView: React.FC<CostsViewProps> = ({ transactions, onNewEntry, onEditTransaction, onDeleteTransaction }) => {
  const [filterMonth, setFilterMonth] = useState('');
  const [filterCategory, setFilterCategory] = useState('');

  // Combine ALL Expenses (type === EXPENSE) 
  // But exclude 'STAFF' entity type (handled in Staff tab)
  // And exclude 'Aluguel Quadra' category (handled in Location tab)
  ...
  const totalDue = costTransactions.reduce((acc, t) => acc + t.amountDue, 0);
  const totalPaid = costTransactions.reduce((acc, t) => acc + t.amountPaid, 0);

  const availableCategories = [...CATEGORY_GROUPS.EXPENSE, ...CATEGORY_GROUPS.COST.filter(c => c !== 'Aluguel Quadra')];

  return (
    <div className="space-y-12 animate-stagger">
      <div className="flex justify-between items-end mb-8">
        <div>
            <h2 className="text-2xl font-light text-white tracking-tight mb-1">General Costs</h2>
            <p className="text-gray-500 font-light text-sm">Variable Expenses & Fees</p>
        </div>
        <button onClick={onNewEntry} className="bg-[#28F587] px-8 py-3 rounded-lg text-black transition-all label-text flex items-center gap-2 shadow-lg shadow-[#28F587]/20 hover:scale-[1.02] text-xs">
            <ICONS.Plus size={18} /> New Expense
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="glass-panel p-8 rounded-2xl relative overflow-hidden group">
             <div className="absolute -right-5 -top-5 p-4 opacity-20 group-hover:opacity-40 transition-opacity"><ICONS.Costs size={100} className="text-[#28F587]" weight="thin" /></div>
             <h3 className="text-[#28F587] label-text text-[10px] mb-2">Total Budgeted</h3>
             <p className="text-4xl value-text text-white tracking-tighter font-mono">
                <CountUp end={totalDue} prefix="R$" />
             </p>
          </div>
          <div className="glass-panel p-8 rounded-2xl">
             <h3 className="text-gray-500 label-text text-[10px] mb-2">Total Paid</h3>
      ...
      <div className="glass-panel rounded-2xl overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-white/5 flex flex-wrap gap-4 items-center bg-white/[0.01]">
            <h3 className="value-text text-lg text-white tracking-tight mr-auto">Expense History</h3>
            ...
            <select 
                ...
                className="toxic-input rounded-lg px-4 py-2 text-xs font-medium outline-none bg-transparent appearance-none"
            >
                <option className="bg-black text-white" value="">All Categories</option>
                {availableCategories.map(c => <option key={c} value={c} className="bg-black text-white">{c}</option>)}
            </select>
        </div>
        <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                    {costTransactions.map(t => (
                        <tr key={t.id} className="hover:bg-white/[0.02] transition-colors group">
                            ...
                            <td className="px-8 py-4 text-white font-medium">
                                {t.entityName} <span className="text-gray-500 text-[10px] block mt-1 label-text">{t.round || t.competitionId || 'N/A'}</span>
                            </td>
                            <td className="px-8 py-4">
                                <span className="bg-white/5 px-2 py-1 rounded text-[10px] label-text border border-white/5">{t.category}</span>
                            </td>
                    ...
                    {costTransactions.length === 0 && <tr><td colSpan={6} className="p-12 text-center text-gray-500 text-xs label-text">Nenhum registro encontrado.</td></tr>}
                </tbody>
            </table>
        </div>
      </div>
